...
def create_encoder_context(
    ...
    codec.options = {
        "profile": "baseline",
        # CHANGED: Otherwise crashes on NVIDIA ENCODER
        "level": "51",
        # "level": "31",
        # CHANGED: Not working with NVIDIA ENCODER
...
class H264Encoder(Encoder):
    ...
    def encode(
        ...
        if H264Encoder.DEBUG_ENCODING.get(f"{frame.pts}", None):
            H264Encoder.DEBUG_ENCODING[f"{frame.pts}"] = H264Encoder.DEBUG_ENCODING[f"{frame.pts}"] + 1
        else:
            H264Encoder.DEBUG_ENCODING[f"{frame.pts}"] = 1
